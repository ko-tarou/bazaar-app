...
export default function Home() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/api/products`, { method: "POST", body })
      if (!res.ok) throw new Error("出品に失敗しました")

      setFormData({ name: "", description: "", price: "", image: null, imagePreview: "" })
      setOpen(false)
      fetchProducts()
    } catch (err) {
      console.error(err)
          ...
          <DialogContent className="sm:max-w-md">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Input id="price" name="price" type="number" min="0" value={formData.price} onChange={handleInputChange} placeholder="未入力で価格未定" />
              </div>
              <div className="space-y-2">
              ...
              <div className="flex justify-end gap-2 pt-2">
                ...
                <Button type="submit" disabled={loading}>{loading ? "送信中..." : "出品する"}</Button>
              </div>
            </form>
          </DialogContent>
        </Dialog>
        ...
        {products.map((product) => (
          <Card key={product.id} className="overflow-hidden h-full flex flex-col">
            ...
            <CardFooter className="border-t p-4 flex justify-between items-center">
              ...
              <Button variant="destructive" size="sm" onClick={() => handleDelete(product.id)}>削除</Button>
            </CardFooter>
          </Card>
        ))}
      </div>
